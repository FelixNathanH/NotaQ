...
class debt extends Home
{
    ...
    public function submit()
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                // For regular products (non-custom), fetch from DB
                if (!($item['is_custom'] ?? false) && !empty($item['product_id'])) {
                    ...
                    // For custom product
                    $productName = $item['custom_name'] ?? 'Produk Kustom';
                    $productPrice = $item['custom_price'] ?? 0;
                }
                $cartModel->insert([
                    'invoice_id'           => $invoice_id,
                    'company_id'           => $company_id,
                    ...
                    'custom_product_price' => $productPrice,
                ]);
    ...
    public function sendReminder()
    {
        ...
        // Fetch company name
        $company = $this->ModelCompany->find($debt['company_id']);
        $companyName = $company ? $company['company_name'] : 'Perusahaan Anda';

        // Fetch invoice items
        $items = $this->ModelCart
            ->where('invoice_id', $debt['invoice_id'])
            ->findAll();
